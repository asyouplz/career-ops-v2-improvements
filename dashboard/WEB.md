...
- 추천·지원현황·지원보류·지원제외를 탭으로 나누고, 탭마다 건수를 표시합니다. 페이지 머리에는 최근 수집 시각, 추천 건수, 새로 발견된 건수를 표시합니다. 수집이 늦어졌다는 안내는 서버가 보내는 경고 목록으로 표시합니다.
- 추천은 사이트별로 묶은 목록입니다. 출처 칩으로 한 사이트만 볼 수 있고, 추천이 없는 사이트는 목록 아래 한 줄로 모읍니다. 최근 수집 기준 24시간 안에 처음 확인된 공고에는 NEW를 표시합니다. 공고 제목을 누르면 원문이 새 창에서 열립니다.
- 지원현황은 진행 중·전체와 단계별 칩으로 거릅니다. 상태 변경 창에서는 단계를 버튼으로 고릅니다.
- 보류·제외는 최근 5개를 먼저 보여 주고, 출처 칩과 `지난 기록 더 보기`, `최근 5개만 보기`를 제공합니다.
- 좁은 화면에서는 탭 이름을 줄이고 검색을 아이콘으로 접습니다. 검색창에서 Escape를 누르면 검색어를 지우고 닫습니다.
...
- 메일 동기화는 상태, 마지막 반영 시각, 확인 필요 건수를 한 줄로 보여 주고 나머지 처리 내역은 펼쳐서 확인합니다. 동기화가 중단되면 상세가 자동으로 펼쳐집니다. 공고 세부 내역에서 메일 근거와 원본 링크를 확인합니다.
