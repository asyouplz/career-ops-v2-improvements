...
- 링크: 공고 제목·세부 내역·푸터처럼 이동하거나 새 창을 여는 요소는 `--link`. 파란 글자는 이 용도로만 씁니다
...
- 버튼 모양: 행의 분류 동작은 `.action-group` 안의 `.group-btn`, 단독 동작은 `.btn-line`으로 같은 테두리·높이를 씁니다. 상세 창을 여는 동작은 `.detail-link`(링크 색 + ›)로 구분합니다.
