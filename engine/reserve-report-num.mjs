...
function occupiedFromReports(reportsDir) {
  ...
  for (const name of readdirSync(reportsDir)) {
    // Date-led analytical reports (for example
    // 2026-07-20-location-audit.md) do not own a tracker/report ID.  Check
    // this shape before the generic numeric prefix so the year is not
    // mistaken for slot 2026 while still allowing real four-digit IDs.
