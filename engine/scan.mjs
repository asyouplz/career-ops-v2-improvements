...
const POSTING_INDEX_PATH = process.env.CAREER_OPS_POSTING_INDEX || 'data/posting-index.tsv';
const PIPELINE_PATH = process.env.CAREER_OPS_PIPELINE || 'data/pipeline.md';
const APPLICATIONS_PATH = 'data/applications.md';
const PROVIDERS_DIR = path.resolve(path.dirname(fileURLToPath(import.meta.url)), 'providers');
const PROJECT_ROOT = path.resolve(process.cwd());
const EXPECTED_PROJECT_ROOT = process.env.CAREER_OPS_EXPECTED_PROJECT_ROOT
  ? path.resolve(process.env.CAREER_OPS_EXPECTED_PROJECT_ROOT)
  : null;
const RUN_ID = process.env.CAREER_OPS_RUN_ID || null;

function assertProjectRootBeforeWrite() {
  if (!EXPECTED_PROJECT_ROOT) return;
  if (PROJECT_ROOT !== EXPECTED_PROJECT_ROOT) {
    throw new Error(`project root mismatch: expected ${EXPECTED_PROJECT_ROOT}, got ${PROJECT_ROOT}`);
  }
  const missing = ['scan.mjs', 'portals.yml', 'data/pipeline.md', 'data/scan-history.tsv']
    .filter(relative => !existsSync(path.join(PROJECT_ROOT, relative)));
  if (missing.length > 0) {
    throw new Error(`project root is missing required files: ${missing.join(', ')}`);
  }
}

// This guard runs before mkdirSync or any scanner write. The wrapper supplies
// EXPECTED_PROJECT_ROOT in production; standalone development remains compatible.
assertProjectRootBeforeWrite();
...
//   - With `check_title: true`, a `block` match in the supplemental job title
//     rejects before location tiers; title evidence never satisfies `allow`
//   - Empty / whitespace-only / non-string location → pass by default; reject
//     when `require_explicit: true` so strict commute boundaries fail closed on
...
export function buildLocationFilter(locationFilter) {
  ...
  const requireExplicit = locationFilter.require_explicit === true;
  const checkTitle = locationFilter.check_title === true;

  return (location, url, title) => {
    // Title is only supplemental negative evidence. It can reject an explicitly
    // blocked work city, but it can never satisfy `allow` or rescue a missing
    // location. This keeps strict commute boundaries fail-closed.
    if (checkTitle && typeof title === 'string') {
      const lowerTitle = title.toLowerCase();
      if (block.length > 0 && block.some((matcher) => matcher(lowerTitle))) return false;
    }

    const lower = typeof location === 'string' ? location.trim().toLowerCase() : '';
    const hint = locationHintFromUrl(url);
    // Nothing to judge on either field → preserve stock pass-through unless the
    // user's config explicitly requires a real location.
    if (lower === '' && hint === '') return !requireExplicit;
 ...
 *
 * Preserve the scanner's established public behavior: trim and lowercase only.
 * The stronger legal-name folding used by V2 lives in the separate identity
 * helpers below so existing scripts that import this canonicalizer keep their
 * key shape.
 *
 * @param {unknown} name - Raw company value from a tracker row or provider job.
 * @returns {string} Stable legal-name key.
 */
function defaultCompanyNormalizer(name) {
  return String(name ?? '').trim().toLowerCase();
}

/** Normalize legal-name formatting for the shared V2 posting identity. */
function identityCompanyNormalizer(name) {
  let value = String(name ?? '').normalize('NFKC').toLowerCase().trim();
  value = value
    .replace(/^\s*(?:\(\s*주\s*\)|주식회사|유한회사)\s*/u, '')
    .replace(/\s*(?:\(\s*주\s*\)|주식회사|유한회사)\s*$/u, '');
  let previous;
  do {
    previous = value;
    value = value.replace(
      /(?:[\s,._-]+|^)(?:co\.?\s*,?\s*ltd\.?|corporation|corp\.?|incorporated|inc\.?|limited|ltd\.?|llc|plc)\s*$/u,
      '',
    ).trim();
...
export function normalizeCompanyForIdentity(name) {
  return identityCompanyNormalizer(name);
}

function buildCanonicalMap(aliases, normalizer) {
  const map = new Map();
  if (aliases && typeof aliases === 'object' && !Array.isArray(aliases)) {
    const entries = Object.entries(aliases);
    const canonicalKeys = new Set();
    for (const [canonical] of entries) {
      const canon = normalizer(canonical);
      if (!canon) continue;
      map.set(canon, canon);
...
function buildCanonicalizer(aliases, normalizer) {
  const map = buildCanonicalMap(aliases, normalizer);
...
/**
 * Extract only structurally delimited company-name segments.
 *
 * This deliberately does not use substring matching. A verified alias may be
 * resolved when it appears as a complete parenthetical or slash-delimited
 * label, while lookalikes such as "예시브랜드페이" remain independent.
 */
function companyIdentitySegments(name) {
  ...
  const add = value => {
    const cleaned = String(value ?? '')
      .replace(/^[\s()[\]{}]+|[\s()[\]{}]+$/gu, '')
      .trim();
    if (cleaned) segments.add(cleaned);
  };
  const addDelimited = value => {
    for (const part of String(value ?? '').split(/\s*(?:\/|｜|\|)\s*/u)) add(part);
  };

  for (const match of raw.matchAll(/\(([^()]*)\)/gu)) addDelimited(match[1]);
  addDelimited(raw.replace(/\([^()]*\)/gu, ' '));
  return [...segments];
}

/**
 * Resolve a V2 company identity with explicit evidence about the match mode.
 * Exact whole-label aliases win. Composite fallback is accepted only when
 * every matched segment points to one unique canonical company.
 */
export function buildCompanyIdentityResolver(aliases) {
  const map = buildCanonicalMap(aliases, identityCompanyNormalizer);
  return function resolveCompanyIdentity(name) {
    const key = identityCompanyNormalizer(name);
    ...
    for (const segment of companyIdentitySegments(name)) {
      const target = map.get(identityCompanyNormalizer(segment));
...
/** Build the stronger legal-name canonicalizer used by V2 and posting IDs. */
export function buildCompanyIdentityCanonicalizer(aliases) {
  const resolveCompanyIdentity = buildCompanyIdentityResolver(aliases);
  return name => resolveCompanyIdentity(name).company_key;
}
 ...
 * name itself resolve to the normalized canonical name. This closes the gap
 ...
 *
 * Unknown names pass through the same legal-name normalization.
 *
...
export function buildCompanyCanonicalizer(aliases) {
  return buildCanonicalizer(aliases, defaultCompanyNormalizer);
}
...
/** Collapse formatting-only separators for V2 posting identity. */
export function normalizeRoleForIdentity(role) {
  return normalizeTextKey(normalizeRoleForDedup(role));
}

/** Build the company+role key used by V2 and the posting sidecar index. */
export function companyRoleIdentityKey(company, role, canonicalize = identityCompanyNormalizer) {
  return `${canonicalize(company)}::${normalizeRoleForIdentity(role)}`;
}
...
export function collectSeenCompanyRoles(
  ...
  roleNormalizer = normalizeRoleForDedup,
) {
  ...
  const add = (company, role) => {
    ...
    seen.add(`${canonicalize(c)}::${roleNormalizer(r)}`);
  };
...
export function loadSeenCompanyRoles(
  ...
  }, policy, canonicalize, roleNormalizer);
}
...
export function formatScanHistoryRow(offer, date, status = 'added') {
  return [
    ...
    // Normalized company key: the canonical legal-name form used by scanner and
    // V2 identity resolution. Stored at write time so
    ...
    offer.companyKey || identityCompanyNormalizer(offer.company || ''),
...
export function appendToScanHistory(offers, date, status = 'added', policy = {}) {
  ...
  // Keep the historical one-row-per-URL/status behavior unless a recheck policy
  // explicitly makes the previous observation stale.  Once stale, one new row
  // records today's outcome and re-arms dedup for the configured interval.
  const existingPairs = new Set();
  for (const line of readFileSync(SCAN_HISTORY_PATH, 'utf-8').split('\n')) {
    const [url, firstSeen, , , , existingStatus] = line.split('\t');
    if (!url || url === 'url') continue;
    const normalizedStatus = existingStatus || 'added';
    if (shouldDedupScanHistoryRow({ firstSeen, status: normalizedStatus }, policy)) {
      existingPairs.add(`${normalizeUrlForDedup(url)}::${normalizedStatus}`);
    }
  }
  const firstOnly = [];
  for (const offer of offers) {
    const pair = `${normalizeUrlForDedup(offer.url)}::${status}`;
    if (existingPairs.has(pair)) continue;
    existingPairs.add(pair);
    firstOnly.push(offer);
  }
  if (firstOnly.length === 0) return;

  const lines = firstOnly.map(o => formatScanHistoryRow(o, date, status)).join('\n') + '\n';

  appendFileSync(SCAN_HISTORY_PATH, lines, 'utf-8');

  if (!existsSync(POSTING_INDEX_PATH)) {
    mkdirSync(path.dirname(POSTING_INDEX_PATH), { recursive: true });
    writeFileSync(
      POSTING_INDEX_PATH,
  ...
  const indexed = new Set(
    readFileSync(POSTING_INDEX_PATH, 'utf-8')
      .split('\n')
      .slice(1)
  ...
  const indexRows = [];
  for (const offer of firstOnly) {
    const normalizedUrl = normalizeUrlForDedup(offer.url);
    const companyKey = offer.companyKey || identityCompanyNormalizer(offer.company);
    const roleKey = offer.roleKey || normalizeRoleForIdentity(offer.title);
    const postingKey = `${companyKey}::${roleKey}`;
    const listingId = `li_${createHash('sha256').update(normalizedUrl).digest('hex').slice(0, 20)}`;
    if (indexed.has(listingId)) continue;
    indexed.add(listingId);
    const clusterId = `pc_${createHash('sha256').update(postingKey).digest('hex').slice(0, 20)}`;
    indexRows.push([
      listingId,
      clusterId,
      normalizeScanUrl(offer.url),
  ...
  if (indexRows.length > 0) {
    appendFileSync(POSTING_INDEX_PATH, `${indexRows.join('\n')}\n`, 'utf-8');
  }
}
...
async function verifyOffers(offers, { headedFallback = false, throttleBaseMs = 0, rediscover = false } = {}) {
  ...
  try {
    ...
    ({ checkLivenessViaApi } = await import('./liveness-api.mjs'));
  } catch (err) {
    throw new Error(
  ...
  // Three permanent buckets + one transient retry bucket:
  //   verified  → only pages positively classified as active
  ...
  try {
    ...
    for (let i = 0; i < offers.length; i++) {
      ...
      const apiResult = await checkLivenessViaApi(offer.url);
      const { result, code, reason } = apiResult || (headed
      ...
      if (result === 'expired') {
        ...
        if (rediscover && code === 'http_gone' && offer.tracked && offer.careersUrlDomain) {
          ...
          if (newUrl) {
            ...
            const apiRecheck = await checkLivenessViaApi(newUrl);
            const recheck = apiRecheck || (headed
        ...
        // Transient network/timeout outcomes are not persisted, so discovery can
        // retry them on the next run without treating ambiguity as active.
        uncertain.push({ ...offer, code, reason });
        console.log(`  ⚠️ uncertain ${offer.company} | ${offer.title} (${reason})`);
      }
  ...
  return { verified, expired, dropped, invalid, migrated, uncertain };
}
...
async function main() {
  ...
  const seenCompanyRoles = loadSeenCompanyRoles(APPLICATIONS_PATH, canonicalizeCompany, {
    ...
    roleNormalizer: normalizeRoleForIdentity,
  });
  ...
  function sourceStats(sourceId) {
    ...
    return sourceResults[id];
  }
  for (const target of targets) sourceStats(target._provider?.id).targets++;
  ...
  const tasks = targets.map(company => async () => {
    ...
    const ctx = {
      ...
      includeUndated: true,
      // Providers that must fetch a detail endpoint for location/status may use
      // this to avoid detail requests for titles the scanner would drop anyway.
      acceptTitle: titleFilter,
    };
    ...
    try {
      ...
      const providerStats = sourceStats(provider.id);
      totalFound += jobs.length;
      providerStats.found += jobs.length;
      ...
      for (const job of jobs) {
        ...
        if (blacklist.size > 0) {
          ...
          if (blEntry) {
            if (!includeBlacklisted) {
              ...
              providerStats.filtered_blacklist++;
              continue;
            }
        ...
        const roleKey = normalizeRoleForIdentity(job.title);
        ...
        if (cooldownResult.skip) {
          ...
          providerStats.filtered_cooldown++;
          cooldownOffers.push({
            job: { ...job, source: sourceName, companyKey, roleKey },
        ...
        providerStats.precheck_candidates++;
      }
    } catch (err) {
      errors.push({
        company: company.name,
  ...
  if (verify && newOffers.length > 0) {
    console.log(`\nVerifying liveness of ${newOffers.length} new offer(s) via structured APIs with Playwright fallback (sequential)...`);
    ...
    uncertainOffers = result.uncertain;
  ...
  if (verify) {
    const livenessSourceId = offer => String(
      offer?._sourceId || offer?.source || 'unknown',
    ).replace(/-api$/, '');
    for (const offer of verifiedOffers) sourceStats(livenessSourceId(offer)).active++;
    for (const offer of expiredOffers) sourceStats(livenessSourceId(offer)).expired++;
    for (const offer of droppedOffers) sourceStats(livenessSourceId(offer)).no_apply_control++;
    for (const offer of uncertainOffers) sourceStats(livenessSourceId(offer)).uncertain++;
    for (const offer of invalidOffers) sourceStats(livenessSourceId(offer)).invalid++;
  }
  ...
  if (!dryRun && verifiedOffers.length > 0) {
    ...
    appendToScanHistory(verifiedOffers, date, 'added', historyPolicy);
  }
  if (!dryRun && cooldownOffers.length > 0) {
    ...
    for (const [status, group] of Object.entries(cooldownGroups)) {
      appendToScanHistory(group, date, status, historyPolicy);
    }
  }
  ...
  if (!dryRun && expiredForHistory.length > 0) {
    appendToScanHistory(expiredForHistory, date, 'skipped_expired', historyPolicy);
  }
  ...
  if (!dryRun && droppedOffers.length > 0) {
    appendToScanHistory(droppedOffers, date, 'skipped_no_apply_control', historyPolicy);
  }
  ...
  if (verify) {
    ...
    console.log(`Uncertain (retry):     ${uncertainOffers.length} deferred`);
  }
  ...
  console.log(`SCAN_SUMMARY_JSON=${JSON.stringify({
    run_id: RUN_ID,
    project_root: PROJECT_ROOT,
    expected_project_root: EXPECTED_PROJECT_ROOT,
    root_verified: EXPECTED_PROJECT_ROOT ? PROJECT_ROOT === EXPECTED_PROJECT_ROOT : null,
    dry_run: dryRun,
    writes_enabled: !dryRun,
    companies_scanned: summaryCompanies,
    job_boards_scanned: summaryBoards,
    total_jobs_found: totalFound,
    filtered_by_title: totalFilteredTitle,
    filtered_by_location: totalFilteredLocation,
    duplicates: totalDupes,
    active_new_offers_added: verifiedOffers.length,
    expired_dropped: expiredOffers.length,
    uncertain_deferred: uncertainOffers.length,
    source_results: sourceResults,
    errors: errors.length,
  })}`);

  console.log('\nReview the collected pipeline with the V2 commands documented in README.md.');
